...
# Support decoupling the API order names with those used by Elasticsearch.
_SORT_BY_MAP = {
    logs_repository.SORT_BY_ORDERS['ascending']: 'asc',
    logs_repository.SORT_BY_ORDERS['descending']: 'desc'
}

# Currently only timestamp is a valid field. This may be extended
# with other fields and dimensions in the future.
_VALID_SORT_BY_FIELDS = {'timestamp'}


class LogsRepository(logs_repository.AbstractLogsRepository):
    def __init__(self):
        # Disable the trace logger to prevent error about missing handler
        # See: https://review.openstack.org/#/c/402147
    ...
    def list_logs(self, tenant_id, region, dimensions, start_time, end_time,
        ...
        search_kwargs = LogsRepository._build_list_logs(
            tenant_id, region, dimensions, start_time, end_time,
            offset, limit, sort_by)
        try:
    ...
    @staticmethod
    def _build_list_logs(tenant_id, region, dimensions, start_time, end_time,
                         offset, limit, sort_by):
        ...
        body = LogsRepository._get_search_query_body(
            region, dimensions, start_time, end_time, offset, limit, sort_by)
    ...
    @staticmethod
    def _get_search_query_filters(region, dimensions, start_time, end_time):
        """Build filter clauses based on dimensions and time range."""

        filters = [LogsRepository._get_region_term_clause(region)]

        if dimensions:
            filters += [LogsRepository._get_dimension_terms_clause(d) for d in
                        dimensions]
    ...
    def _get_search_query_body(
        ...
        # Construct the query clause. There will always be at least one for
        # the region
        filters = LogsRepository._get_search_query_filters(
            region, dimensions, start_time, end_time)
        body[u'query'] = {u'bool': {u'filter': filters}}

        # Add sort_by field if supplied.
        if sort_by:
            body[u'sort'] = [LogsRepository._get_sort_by_clause(s) for s in
                             sort_by]

        return body

    @staticmethod
    def _get_dimension_terms_clause(dimension):
    ...
    @staticmethod
    def _get_region_term_clause(region_name):
        """Construct a term clause for the region."""
        return {u'term': {cfg.CONF.elasticsearch.region_field: region_name}}

    @staticmethod
    def _get_sort_by_clause(sort):
        """Construct a sort clause to pass in the search body."""

        assert (sort.field in _VALID_SORT_BY_FIELDS)
        field = cfg.CONF.elasticsearch.timestamp_field

        # Map the base class sort direction naming to Elasticsearch speak.
        direction = _SORT_BY_MAP[sort.direction]

        return {field: {u'order': direction}}
